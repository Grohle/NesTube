...
from nestube.ui_qt.forms.ui_tab_nesting import Ui_TabNesting
...
class TabNesting(QWidget):
    ...
    def _place_floating_piece(self, scene_pos: QPointF) -> None:
        ...
        if not is_move:
            try:
                from nestube.dxf_cache import auto_generate_and_save, piece_dxf_path
    ...
    def _open_cut_piece_in_profile_creator(self, corte: Corte) -> None:
        ...
        from nestube.ui_qt.dialogs.cut_piece_dialog import CutPieceDialog
        H = self._section_height_mm()
    ...
    def _synth_profile_meta(self, profile_name: str) -> dict:
        ...
        """
        from nestube.models import TipoPerfil
    ...
    def _export_sidebar_piece_dxf(self, pi: PieceInfo) -> None:
        ...
        from PySide6.QtWidgets import QFileDialog
        import shutil
        from nestube.dxf_cache import auto_generate_and_save, piece_dxf_path, save_piece_contour
        profile_name = self._current_profile_name()
        corte = pi.corte
        H = self._section_height_mm()
    ...
    def _change_piece_values(self, pi: PieceInfo) -> None:
        ...
        from nestube.ui_qt.dialogs.change_values_dialog import ChangeValuesDialog
    ...
    def _open_material_search(self) -> None:
        ...
        from nestube.ui_qt.dialogs.stock_material_search_dialog import (
    ...
    def _deduct_stock_bars(self) -> None:
        ...
        from nestube.stock_db import deduct_bar, restore_bar
    ...
    def _export_nesting_pdf(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, t("export_pdf"), "nestube_nesting.pdf", "PDF (*.pdf)")
        if not path:
            return
        try:
            from nestube.nesting_pdf import export_multi_nesting_pdf
    ...
    def _export_nesting_dxf(self) -> None:
        ...
        if nestings is None:
            return
        path, _ = QFileDialog.getSaveFileName(
            self, t("export_dxf"), "nestube_nesting.dxf", "DXF (*.dxf)")
        if not path:
            return
        try:
            from nestube.nesting_dxf import export_multi_nesting_dxf
    ...
    def _export_nesting(self) -> None:
        ...
        if rect.isEmpty():
            ...
            return
        path, _ = QFileDialog.getSaveFileName(
            self, t("export_nesting_png"), "nestube_nesting.png", "PNG (*.png)")
        if not path:
            return
