"""
nestube/ui_qt/dialogs/profile_save_dialog.py
...
from nestube.ui_qt.forms.ui_profile_save_dialog import Ui_ProfileSaveDialog
...
class ProfileSaveDialog(QDialog):
    ...
    def _setup_name_completer(self) -> None:
        names: list[str] = []
        for cp in __import__("nestube.app_config", fromlist=["get"]).get().custom_profiles:
            if cp.name:
