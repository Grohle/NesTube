...
from nestube.stock_db import get_stock, save_stock, remove_bar, add_bar, update_bar, StockBar
import nestube.ui_qt.theme_qt as _th
from nestube.ui_qt.icons import themed_icon

_log = logging.getLogger(__name__)
from nestube.ui_qt.forms.ui_tab_stock import Ui_TabStock
...
class TabStock(QWidget):
    ...
    def _rebuild_profile_combo(self) -> None:
        ...
        try:
            from nestube.stock_db import get_profiles_in_stock
    ...
    def _open_stock_dialog(self, bar: Optional[StockBar] = None) -> None:
        try:
            from nestube.ui_qt.dialogs.stock_add_dialog import StockAddDialog
        except ImportError:
    ...
    def _export_stock(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, t("export_excel"), "nestube_stock.xlsx", "Excel (*.xlsx)",
        )
        if not path:
            return
        try:
            from nestube.stock_export import export_stock_to_excel
