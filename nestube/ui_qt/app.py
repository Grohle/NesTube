"""
nestube/ui_qt/app.py
NesTubeApp — QMainWindow, menu bar, QTabWidget, shared AppState.
"""
...
from nestube.i18n import available_languages, set_language, t
...
from nestube.ui_qt.tab_stock import TabStock
from nestube.ui_qt.theme_qt import apply_theme, build_palette, build_stylesheet
...
class NesTubeApp(QMainWindow):
    ...
    def _on_use_stock_toggled(self, checked: bool) -> None:
        ...
        if not checked:
            ...
            return
        from nestube.stock_prefill import prefill_active_material_from_stock
        from nestube.bevel_geom import profile_available_heights
    ...
    def _configure_opt_times(self) -> None:
        ...
        from nestube.ui_qt.dialogs.optimization_times_dialog import OptimizationTimesDialog
    ...
    def _configure_cost_defaults(self) -> None:
        ...
        from nestube.ui_qt.dialogs.cost_defaults_dialog import CostDefaultsDialog
    ...
    def _open_edit_templates(self) -> None:
        from nestube.ui_qt.dialogs.pdf_template_editor import EditTemplatesDialog
        EditTemplatesDialog(self).exec()

    def _configure_nesting_layout(self) -> None:
        from nestube.ui_qt.dialogs.nesting_layout_dialog import NestingLayoutDialog
    ...
    def _configure_naming(self) -> None:
        from nestube.ui_qt.dialogs.naming_dialog import NamingDialog
    ...
    def _open_backups(self) -> None:
        from nestube.ui_qt.dialogs.backup_dialog import BackupDialog
        BackupDialog(self).exec()

    def _open_db_management(self) -> None:
        from nestube.ui_qt.dialogs.database_management_dialog import DatabaseManagementDialog
    ...
    def _importar_excel(self) -> None:
        ...
        if not path:
            return
        try:
            from nestube.excel_import import import_cuts_from_excel
    ...
    def _save_import_template(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            self, t("save_template"), "nestube_import_template.xlsx", "Excel (*.xlsx)",
        )
        if not path:
            return
        try:
    ...
    def _exportar_xlsx(self) -> None:
        ...
        if not state.cortes:
            ...
            return
        path, _ = QFileDialog.getSaveFileName(
            self, t("export_xlsx"), "nestube_cuts.xlsx", "Excel (*.xlsx)",
        )
        if not path:
            return
        try:
    ...
    def _exportar_pdf(self) -> None:
        from nestube.export_utils import exportar_pdf, _next_number
        from nestube.context_sync import effective_barras, recompute_auto_barras
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, t("export_pdf"),
            f"NesTube_Anidado_{_next_number(initial_dir, 'pdf')}.pdf",
            "PDF (*.pdf)",
        )
        if not path:
            return
    ...
    def _github_url(self) -> str:
        ...
        if not url or "example.com" in url:
            return "https://github.com/Grohle/nestube"
        return url

    def _show_about(self) -> None:
        from nestube.ui_qt.dialogs.about_dialog import show_about_dialog
        show_about_dialog(
    ...
    def _show_about_startup(self) -> None:
        from nestube.ui_qt.dialogs.about_dialog import AboutDialog
