"""
nestube/nestube/nestube/context_sync.py
...
def nesting_layout_piece_count(nesting_layout: list) -> int:
    # Count placed pieces directly — no need to sort each bar or coerce lengths
    # to float the way layout_to_barras does just to take its length.
    return sum(len(bar) for bar in (nesting_layout or []) if bar)
