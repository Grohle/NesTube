...
def _ffd_pack(longitud_barra: float, lista: List[float]) -> List[List[float]]:
    ...
    while remaining:
        ...
        for corte in sorted(remaining, reverse=True):
            if corte <= restante:
                barra.append(corte)
                restante -= corte
        if not barra:
            ...
            # alone instead of breaking, which would silently drop that piece —
            # and, with this loop structure, every remaining piece after it.
            biggest = max(remaining)
            remaining.remove(biggest)
            barras.append([biggest])
            continue
        for c in barra:
            remaining.remove(c)
        barras.append(barra)
