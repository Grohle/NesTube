...
from nestify.ui_qt.forms.ui_tab_perfiles import Ui_TabPerfiles
...
from nestify.ui_qt.widgets.stock_material_search_bar import StockMaterialSearchBar
...
class TabPerfiles(QWidget):
    ...
    def _state_for_export(self):
        ...
        import copy
        from nestify.context_sync import ensure_material_contexts, effective_barras
    ...
    def _export_excel(self) -> None:
        from nestify.export_utils import exportar_excel, _next_number
        initial_dir = self._state.export_path or "."
        path, _ = QFileDialog.getSaveFileName(
            self, t("export_excel"),
            f"Nestify_Resultados_{_next_number(initial_dir, 'xlsx')}.xlsx",
            "Excel (*.xlsx)",
        )
        if not path:
            return
    ...
    def _export_pdf(self) -> None:
        from nestify.export_utils import exportar_presupuesto_pdf, _next_number
        initial_dir = self._state.export_path or "."
        path, _ = QFileDialog.getSaveFileName(
            self, t("export_pdf"),
            f"Nestify_Presupuesto_{_next_number(initial_dir, 'pdf')}.pdf",
            "PDF (*.pdf)",
        )
        if not path:
            return
    ...
    def _export_docx(self) -> None:
        from nestify.export_utils import exportar_docx, _next_number
        initial_dir = self._state.export_path or "."
        path, _ = QFileDialog.getSaveFileName(
            self, t("export_docx"),
            f"Nestify_Presupuesto_{_next_number(initial_dir, 'docx')}.docx",
            "Word DOCX (*.docx)",
        )
        if not path:
            return
    ...
    def _print(self) -> None:
        try:
            from nestify.export_utils import imprimir
