...
def exportar_excel(state: AppState, filename: Optional[str] = None) -> None:
    ...
    if filename is None:
        ...
        filename = filedialog.asksaveasfilename(
            ...
            initialfile=f"Nestify_Resultados_{_next_number(initial_dir, 'xlsx')}.xlsx",
...
def exportar_presupuesto_pdf(state: AppState, filename: Optional[str] = None) -> None:
    ...
    if filename is None:
        ...
        filename = filedialog.asksaveasfilename(
            defaultextension=".pdf",
            filetypes=[("PDF", "*.pdf")],
            initialfile=f"Nestify_Presupuesto_{_next_number(initial_dir, 'pdf')}.pdf",
            initialdir=initial_dir,
        )
        if not filename:
            return
...
def exportar_docx(state: AppState, filename: Optional[str] = None) -> None:
    ...
    if filename is None:
        ...
        filename = filedialog.asksaveasfilename(
            ...
            initialfile=f"Nestify_Presupuesto_{_next_number(initial_dir, 'docx')}.docx",
            initialdir=initial_dir,
        )
        if not filename:
            return
