...
- More optimization time never yields a worse result and, on a packing-hard
  instance, finds a strictly better one (the iterated-local-search uses the budget).
...
def test_more_time_never_worse_and_can_improve():
    ...
    # Best-kept search: more time can never produce a worse score.
    assert slow <= fast, f"more time regressed: {fast} -> {slow}"
    # On this instance it should find a strictly better packing (fewer bars).
    assert slow[0] <= fast[0]
    assert slow < fast, f"more time did not help on a hard instance: {fast} -> {slow}"
